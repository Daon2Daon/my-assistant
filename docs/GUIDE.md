...
**작업 항목**:
- [x] BackgroundScheduler 초기화 및 FastAPI 라이프사이클 연동
- [x] 스케줄러 상태 관리 (시작/중지/재시작)
- [x] Job 등록/삭제/조회 유틸리티 함수 작성
- [x] 서버 재시작 시 Job 복원 로직 구현
...
**작업 항목**:
- [x] OpenWeatherMap API 클라이언트 구현
- [x] 날씨 데이터 파싱 및 메시지 포맷팅
  - 최저/최고 기온
  - 강수 확률
  - 미세먼지 농도 (별도 API 필요 시 추가)
  - 우산 필요 여부 판단 로직
- [x] 카카오톡 메시지 템플릿 작성
- [x] 정기 발송 Job 등록 (CronTrigger)
...
**작업 항목**:
- [x] Yahoo Finance API 클라이언트 구현 (yfinance 라이브러리)
- [x] PyKRX를 통한 국내 증시 데이터 수집
- [x] 시황 데이터 파싱 및 메시지 포맷팅
  - S&P500, Nasdaq, Dow Jones
  - KOSPI, KOSDAQ
  - 관심 종목 변동률
- [x] 미국/한국 시황 발송 시간 분리 설정
...
**작업 항목**:
- [x] Google Calendar API 클라이언트 구현
- [x] Primary 캘린더 일정 조회 로직
...
- [x] 메모 등록 시 APScheduler Job 동적 추가
  - DateTrigger를 사용한 일회성 작업
- [x] 메모 발송 후 `is_sent` 상태 업데이트
- [x] 서버 재시작 시 미발송 메모 Job 복원 로직
...
**작업 항목**:
- [x] Jinja2 템플릿 설정 및 기본 레이아웃 작성
- [x] CSS 프레임워크 적용 (Tailwind CSS 또는 Bootstrap)
- [x] 대시보드 메인 페이지
...
**작업 항목**:
- [x] 카카오 로그인 버튼 및 연동 상태 표시
- [x] 구글 로그인 버튼 및 연동 상태 표시
- [x] 토큰 만료 경고 표시
...
**작업 항목**:
- [x] Dockerfile 작성
- [x] docker-compose.yml 작성 (로컬 테스트용)
- [x] docker-compose.prod.yml 작성 (서버 배포용, Watchtower 지원)
- [x] 볼륨 마운트 설정 (DB 파일 보존)
- [x] 환경변수 주입 설정 (APP_PORT 등)
- [x] .dockerignore 작성
...
**작업 항목**:
- [x] Docker 컨테이너 설정 완료
- [x] Watchtower 자동 업데이트 레이블 적용
- [x] 로그 로테이션 설정 (json-file, 10MB x 3)
- [ ] 로컬 서버 또는 NAS 배포
- [ ] 백업 스크립트 작성 (DB 파일)
...
- [x] Phase 3: 날씨/금융 정기 알림 발송 확인
- [x] Phase 4: 캘린더/메모 알림 발송 확인
- [x] Phase 5: Web UI 정상 동작 확인
