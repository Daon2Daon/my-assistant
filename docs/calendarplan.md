...
### Phase C-1: 캘린더 목록 조회
- [x] `get_calendar_list()` 함수 구현
- [x] GET `/api/calendar/list` 엔드포인트
- [x] 응답 스키마 정의

### Phase C-2: 캘린더 선택 저장
- [x] 캘린더 선택 저장 로직
- [x] POST `/api/calendar/select` 엔드포인트
- [x] GET `/api/calendar/selected` 엔드포인트

### Phase C-3: 다중 캘린더 일정 조회
- [x] `get_multiple_calendars_events()` 함수 구현
- [x] 기존 `get_calendar_events()` 수정
- [x] 캘린더별 일정 분류
...
- **Phase C-1**: 2026-01-19 완료 (캘린더 목록 조회)
- **Phase C-2**: 2026-01-19 완료 (캘린더 선택 저장)
- **Phase C-3**: 2026-01-19 완료 (다중 캘린더 일정 조회)
- **Phase C-4**: 2026-01-19 완료 (알림 메시지 개선)
- **Phase C-5**: 2026-01-19 완료 (UI 개선)
- **Phase C-8**: 2026-01-19 완료 (테스트 및 문서화)
...
- 다중 캘린더 선택 및 저장 (DB 영속화)
- 선택 상태 복원 (페이지 로드 시 자동 체크)
- 캘린더별 색상 및 권한 정보 표시

#### ✅ 일정 조회
- 단일 캘린더 조회 (Primary only - 기존 기능)
- 다중 캘린더 동시 조회 (선택된 캘린더 모두)
- 캘린더별 일정 그룹화 및 표시
- 하위 호환성 유지 (캘린더 선택 안 한 경우 Primary만 조회)

#### ✅ 알림 메시지 상세화
- 📍 위치 정보 표시
- 🔗 Google Meet/화상회의 링크 표시
- 👥 참석자 정보 표시 (최대 3명, 초과 시 "외 N명")
- 요일 표시 추가 (예: "2026년 01월 19일 (일)")
...
| 파일 | 상태 | 변경 내용 | 코드량 |
|------|------|----------|--------|
| `services/auth/google_auth.py` | 수정 | `get_calendar_list()`, `get_multiple_calendars_events()` 추가 | +120줄 |
| `services/bots/calendar_bot.py` | 수정 | 다중 캘린더 조회, 상세 메시지 포맷팅 | +180줄 |
| `routers/calendar.py` | 수정 | `/list`, `/select`, `/selected` API 추가 | +120줄 |
| `templates/calendar.html` | 수정 | 캘린더 선택 섹션 추가 | +30줄 |
| `static/js/calendar.js` | 수정 | `loadCalendarList()`, `saveCalendarSelection()` 추가 | +130줄 |
| `docs/calendar_test_guide.md` | 신규 | 테스트 가이드 문서 | +500줄 |
| **합계** | **1개 신규, 5개 수정** | **Phase C-1 ~ C-5 완료** | **~1,080줄** |

### 13.4 API 엔드포인트 추가

| Method | Endpoint | 설명 | Phase |
|--------|----------|------|-------|
| GET | `/api/calendar/list` | 사용자 캘린더 목록 조회 | C-1 |
| POST | `/api/calendar/select` | 선택 캘린더 저장 | C-2 |
| GET | `/api/calendar/selected` | 선택된 캘린더 조회 | C-2 |

### 13.5 데이터베이스 스키마

**옵션 B 선택**: 기존 `Setting` 테이블의 `config_json` 필드 활용

```sql
-- Setting 테이블 (기존)
-- category = 'calendar'일 때 config_json 구조:
{
  "selected_calendars": [
    {
      "id": "primary",
...
- ✅ 테스트 가이드 문서 작성 완료 (`docs/calendar_test_guide.md`)
- ⏳ 사용자 수동 테스트 필요 (로컬 환경)

**테스트 항목**:
- Phase C-1: 캘린더 목록 조회
- Phase C-2: 캘린더 선택 저장/복원
- Phase C-3: 다중 캘린더 일정 조회
- Phase C-4: 상세 정보 표시 (위치/링크/참석자)
- Phase C-5: UI 동작
- 통합 테스트: 신규/기존 사용자 시나리오
- 에러 처리: Google 연동 없음, 토큰 만료 등

### 13.8 미구현 기능 (선택 사항)

#### Phase C-6: 일정 상세 정보 (선택)
- 일정 상세 조회 API
- 화상회의 링크 추출 (Zoom, Teams 등)
- 상세 정보 모달 UI

#### Phase C-7: 일정 생성 기능 (선택)
- OAuth 스코프 확장 필요 (`calendar.readonly` → `calendar`)
- 기존 사용자 재인증 필요
- `create_event()` 함수 구현
- 일정 생성 폼 UI
...
1. **메시지 길이 제한**
   - 카카오톡: 2,000자 제한 (권장: 캘린더 3-5개, 일정 10개 이하)
   - 텔레그램: 4,096자 제한 (권장: 캘린더 5-10개)

2. **캘린더 삭제 처리**
   - Google에서 캘린더 삭제 시 자동 제거 안 됨
   - 수동으로 선택 해제 필요

3. **성능**
   - 캘린더 5개, 일정 50개 기준: ~2초 내 조회
   - Google Calendar API 제한: 1,000,000 요청/일 (충분함)

### 13.10 다음 단계 제안

1. **사용자 테스트 진행**
   - `docs/calendar_test_guide.md` 참고
   - 실제 Google Calendar 데이터로 검증

2. **선택 기능 구현 검토**
   - Phase C-6: 일정 상세 정보 (필요시)
   - Phase C-7: 일정 생성 기능 (필요시)
...
**작성일**: 2026-01-19
**최종 업데이트**: 2026-01-19
**상태**: ✅ **MVP 구현 완료** (Phase C-1 ~ C-5, C-8)
**버전**: 1.0.0
